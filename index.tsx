...
const MessageBubble = ({ message, onImageClick, onSetReply, onSetEdit, isFromAttendant = false, messageIndex, isHighlighted = false }) => {
  ...
  const bubbleClasses = isBot ? 'bg-white text-gray-800 self-start' : isAttendant ? 'bg-blue-100 text-gray-800 self-end' : 'bg-[#dcf8c6] text-gray-800 self-end';

  return (
    <div id={`message-${messageIndex}`} className={`flex w-full ${isBot ? 'justify-start' : 'justify-end'} group items-center`}>
      <div className={`relative flex items-center ${isAttendant ? 'flex-row-reverse' : ''}`}>
        <div className={`max-w-md md:max-w-lg p-2 rounded-lg shadow-sm mb-1 flex flex-col ${bubbleClasses} ${highlightClass}`}>
          {message.replyTo && (
          ...
          <div className="text-xs text-gray-400 self-end mt-1 flex items-center">{message.edited && <span className="mr-1 italic">editada</span>}{new Date(message.timestamp).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</div>
        </div>
        {!isSystem && (
    ...
    <div className="flex-1 flex flex-col bg-gray-100">
      <header className="bg-white p-3 border-b flex justify-between items-center shadow-sm">
        ...
        <div className="flex items-center space-x-2">
            ...
            {chatType === 'human' && attendant?.id === selectedChat.attendantId && <button onClick={() => setTransferModalOpen(true)} className="px-3 py-1 text-xs text-white bg-blue-600 rounded hover:bg-blue-700">Transferir</button>}
          ...
          <footer className="bg-gray-200 p-3">
            {editingMessage ? (
              ...
              <>
                ...
                <div className="flex items-center bg-white rounded-full px-2 shadow">
                  <button onClick={() => fileInputRef.current.click()} className="p-2 text-gray-500"><svg className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15.172 7l-6.586 6.586a2 2 0 102.828 2.828l6.414-6.586a4 4 0 00-5.656-5.656l-6.415 6.585a6 6 0 108.486 8.486L20.5 13" /></svg></button>
                  <input type="file" ref={fileInputRef} onChange={onFileSelect} className="hidden" multiple />
                  <input type="text" value={message} onChange={e => setMessage(e.target.value)} onKeyPress={e => e.key === 'Enter' && handleSend()} placeholder="Digite sua mensagem..." className="w-full p-2 bg-transparent outline-none" />
                  <button onClick={handleSend} className="p-2 text-blue-600"><svg className="h-6 w-6" viewBox="0 0 20 20" fill="currentColor"><path d="M10.894 2.553a1 1 0 00-1.788 0l-7 14a1 1 0 001.169 1.409l5-1.429A1 1 0 009 15.571V11a1 1 0 112 0v4.571a1 1 0 00.725.962l5 1.428a1 1 0 001.17-1.408l-7-14z" /></svg></button>
                </div>
              </>
            )}
          </footer>
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      if (selectedChat) {
          ...
          if (updated && updated.lastMessage?.timestamp !== selectedChat.messageLog[selectedChat.messageLog.length-1]?.timestamp) {
